...
import type { SharedAssessmentResults } from "@/types";
...
export function SharedResultsPage() {
  ...
  const resultsQuery = useQuery({
    ...
    queryFn: () => api.get<SharedAssessmentResults>(`/shared-results/${token}`)
  });
  ...
  if (resultsQuery.isLoading) {
    return <div className="p-8 text-sm text-muted-foreground">Loading shared report...</div>;
  }

  if (!results) {
    return <div className="p-8 text-sm text-muted-foreground">Shared report is unavailable.</div>;
  }
    ...
    <div className="min-h-screen bg-background px-4 py-8">
      <div className="mx-auto max-w-6xl space-y-6">
        <section className="rounded-[2rem] border bg-[radial-gradient(circle_at_top_left,_rgba(114,191,68,0.22),_transparent_38%),linear-gradient(135deg,_rgba(255,255,255,0.98),_rgba(238,248,232,0.96))] px-6 py-8 shadow-sm">
          <div className="text-xs font-semibold uppercase tracking-[0.24em] text-primary">Shared results</div>
          ...
          <Card>
            <CardContent className="p-5">
              ...
              <div className="mt-3 text-xl font-semibold">
                {results.submittedAt
                  ? new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric" }).format(new Date(results.submittedAt))
                  : "-"}
              </div>
            </CardContent>
          </Card>
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr,1fr]">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Lowest-scoring questions in the current submitted report.</CardDescription>
            </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Question-by-question submitted selections for this shared report.</CardDescription>
          </CardHeader>
